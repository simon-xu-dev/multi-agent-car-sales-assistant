...
- **解决方案**：补丁 `tools/ops/patch_mention_filter.py` —— 在 `_was_mentioned` 开头加入协议消息豁免：消息体含 `TASK_COMPLETED/TASK_RECEIVED/TASK_FAILED` 直接放行。已应用到 Leader 与全部 8 个 Worker
- **验证**：DEAL-2003 后半程 intent-analyst→strategy-planner→negotiation-executor→…→complete_project 全程无人工干预自动流转
- **注意**：Team/Leader 被 manager 重建后容器为全新镜像，补丁丢失，需重新执行 `tools/ops/apply_mention_patch.sh`
...
- **脚本**：`tools/ops/recover_lead_intake.sh`、`tools/ops/recover_strategy_planner.sh`
...
- **解决方案**：`tools/ops/patch_force_global.py` 给 `_get_team_id` 加 override 文件检查（`/root/hiclaw-fs/team_force_global` 存在则强制全局路径）+ `mc mirror` 镜像存量文件 + 重启容器
- **脚本**：`tools/ops/fix_leader_path.sh`（可重复执行）

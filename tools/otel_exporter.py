...
- span_kind 映射：agent→INTERNAL, tool→CLIENT, rag→INTERNAL
...
用法:
    python3 tools/otel_exporter.py                    # 重放 3 个场景的工具调用 Trace
...
def _map_kind(span_kind: str) -> SpanKind:
    """工具网关 span_kind → OTel SpanKind（GenAI 语义：tool=CLIENT，agent/rag=INTERNAL）。"""
    return {
...
def _enrich_attributes(span: dict) -> dict:
    """给工具网关 span 的 attributes 补 GenAI semconv 属性。
...
def _load_trace(path: Path) -> list[dict]:
    ...
    if isinstance(rec, dict):
        return rec.get("G3_unified_trace", rec.get("trace", []))
    return []
...
def main() -> None:
    ...
    for tpath in traces:
        ...
        print(f"[{tpath.name}] 重放 {len(trace_tree)} 个工具网关 span → 真 OTel SDK Span...")
        ...
        print(f"  导出 {summary['spans_exported']} span | OTel trace_id={otel_tid[:16]}... | 原 trace_id={summary['original_trace_id'][:16]}...")
